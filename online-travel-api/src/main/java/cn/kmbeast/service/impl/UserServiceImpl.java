...
public class UserServiceImpl implements UserService {
    ...
    public Result<Object> login(UserLoginDTO userLoginDTO) {
        ...
        String tokenKey = RedisKeyConstants.userTokenKey(user.getId());
        String oldToken = stringRedisTemplate.opsForValue().get(tokenKey);
        if (StringUtils.hasText(oldToken) && !oldToken.equals(token)) {
            blacklistToken(oldToken);
        }
        stringRedisTemplate.opsForValue().set(
                tokenKey,
                token,
                JwtUtil.getTokenExpireSeconds(),
                TimeUnit.SECONDS
        );
    ...
    public Result<String> batchDelete(List<Integer> ids) {
        ...
        ids.forEach(id -> stringRedisTemplate.delete(RedisKeyConstants.userTokenKey(id)));
        invalidateStaticControlsCache();
        return ApiResult.success();
    }
    ...
    public Result<String> updatePwd(Map<String, String> map) {
        ...
        stringRedisTemplate.delete(RedisKeyConstants.userTokenKey(user.getId()));
        return ApiResult.success();
    }
    ...
    public Result<String> logout(String token) {
        ...
        if (claims == null) {
            return ApiResult.error("令牌无效");
        }
        Integer userId = claims.get("id", Integer.class);
        String tokenKey = RedisKeyConstants.userTokenKey(userId);
        String activeToken = stringRedisTemplate.opsForValue().get(tokenKey);
        if (StringUtils.hasText(activeToken) && activeToken.equals(token)) {
    ...
    private void blacklistToken(String token) {
        Claims claims = JwtUtil.fromToken(token);
        if (claims == null || !StringUtils.hasText(claims.getId())) {
            return;
        }
        long remainSeconds = JwtUtil.getRemainingSeconds(claims);
        if (remainSeconds <= 0) {
            return;
        }
        stringRedisTemplate.opsForValue().set(
                RedisKeyConstants.tokenBlacklistKey(claims.getId()),
                "1",
                remainSeconds,
                TimeUnit.SECONDS
        );
